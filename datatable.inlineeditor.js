...
$.fn.dataTable.Api.register("MakeCellsEditable()", function(settings) {
    ...
    switch (inputType) {
      ...
      case "datepicker-confirm":
        ...
        if (typeof $.ui == "undefined") {
          alert(
            "jQuery UI is required for the DatePicker control but it is not loaded on the page!"
          )
          break
        }
        $(".datepick").datepicker("destroy")
        var $input = $("<input>")
          .attr("id", "ejbeatycelledit")
          .attr("name", "date")
          .attr("type", "text")
          .addClass(inputCss)
          .addClass("datepick")
        ...
        setTimeout(function() {
          ...
          if (
            typeof inputSetting.options !== "undefined" &&
            typeof inputSetting.options.icon !== "undefined"
          ) {
  ...
  function cancelEditableCell(callingElement) {
    var table = $(callingElement.closest("table"))
      .DataTable()
      .table()
    var cell = table.cell($(callingElement).parent())
  ...
  if (table != null) {
    $(table.body()).on("dblclick", "td", function() {
      ...
      if (
        (settings.columns &&
          settings.columns.indexOf(currentColumnIndex) > -1) ||
        !settings.columns
      ) {
        ...
        if (
          !$(cell).find("input").length &&
          !$(cell).find("select").length &&
          !$(cell).find("textarea").length
        ) {
